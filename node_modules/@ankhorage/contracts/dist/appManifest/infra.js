...
import { AUTH_PROFILE_CREATE_STRATEGIES, AUTH_PROFILE_PRIMARY_KEY_STRATEGIES, AUTH_PROFILE_UPDATE_STRATEGIES, AUTH_SCOPES, AUTH_SIGN_IN_IDENTIFIERS, AUTH_SIGN_UP_POLICIES, AUTHZ_ENGINES, AUTHZ_KINDS, DATABASE_TIERS, STATE_PERSISTENCE_MODES, STORAGE_PROVIDERS, } from '../types';
import { isApiDefinitionList } from './apis';
import { isIconSpec } from './icon';
import { isOptionalBoolean, isOptionalString, isRecord, isStringArray, isStringRecord, } from './shared';
...
function isAuthSignInSpec(value) {
    return (isRecord(value) &&
        Array.isArray(value.identifiers) &&
        value.identifiers.every((identifier) => typeof identifier === 'string' && AUTH_SIGN_IN_IDENTIFIER_SET.has(identifier)));
}
function isAuthSignUpSpec(value) {
    return (isRecord(value) &&
...
function isAuthOAuthProviderConfig(value) {
    return (isRecord(value) &&
